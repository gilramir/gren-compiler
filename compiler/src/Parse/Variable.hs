    ...
    let (# pos1, col1 #) = chompUpper pos end col
     in if pos == pos1
          then eerr row col toError
            ...
            let (# status, newPos, newCol #) = moduleNameHelp pos1 end col1
             in case status of
                  Good ->
                    let !name = Name.fromPtr pos newPos
                        !newState = P.State src newPos end indent row newCol
                     in if isAsciiModuleName (Name.toChars name)
  ...
  if isDot pos end
    then
      let !pos1 = plusPtr pos 1
       ...
       in if pos1 == newPos
            then (# Bad, newPos, newCol #)
            else moduleNameHelp newPos end newCol
    else (# Good, pos, col #)
  ...
  let (# newPos, newCol #) = chompUpper pos end col
   in if pos == newPos
        then (# pos, pos, col #)
          ...
          if isDot newPos end
            then foreignUpperHelp (plusPtr newPos 1) end (newCol + 1)
            else (# pos, newPos, newCol #)
...
foreignAlphaHelp pos end col =
  let (# lowerPos, lowerCol #) = chompLower pos end col
   in if pos < lowerPos
        then (# pos, lowerPos, lowerCol, Src.LowVar #)
        else
          let (# upperPos, upperCol #) = chompUpper pos end col
           in if pos == upperPos
                then (# pos, pos, col, Src.CapVar #)
                else
                  if isDot upperPos end
                    then foreignAlphaHelp (plusPtr upperPos 1) end (upperCol + 1)
                    else (# pos, upperPos, upperCol, Src.CapVar #)
