...
optimize cycle (Can.Expr _ region expression) =
  case expression of
    ...
    Can.LetDestruct pattern expr body ->
      do
        (A.At nameRegion name, destructs) <- destruct pattern
  ...
  case expression of
    Can.Call func args ->
      do
        ...
        if isMatchingName
          then case Index.indexedZipWith (\_ a b -> (A.toValue a, b)) argNames oargs of
    ...
    Can.LetDestruct pattern expr body ->
      do
        (A.At dregion dname, destructors) <- destruct pattern
        oexpr <- optimize cycle expr
        obody <- optimizeTail cycle rootName argNames body
        pure $
